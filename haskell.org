...
This says that the =fst= function takes in a pair of type ~(a,b)~ where ~a~ and ~b~ can be of any types. The function
...
What's the motivation behing having Typeclasses? Well, if a type is part of a typeclass, that means that it supports
...
=a= now can not be of /any/ type, but now it /must/ be of a type that is of /typeclass/ =Eq=. =Eq= typeclass provides
an interface for testing for equality. Most types in Haskell are part of =Eq= typeclass because in general, we should
